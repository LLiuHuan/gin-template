...
// Generator 生成器负责生成代码、添加导入、格式化并将其写入文件。
...
// NewGenerator 函数创建生成器的实例，并将输出文件的名称作为参数。
...
func (g *Generator) ParserAST(p *Parser, structs []string) (ret *Generator) {
	for _, v := range structs {
		g.buf[database.ToDBName(v)] = new(bytes.Buffer)
	}
...
func (g *Generator) Generate() *Generator {
	...
	for _, v := range g.structConfigs {
		if _, ok := g.buf[database.ToDBName(v.StructName)]; !ok {
			continue
		}
		if err := outputTemplate.Execute(g.buf[database.ToDBName(v.StructName)], v); err != nil {
			panic(err)
		}
	}
